...
public class Obstacle implements OpticStuff
{
   ...
   {
      if (side == 0)
         return new Obstacle(getRow(), getColumn(), number, game);
      else
         return new Obstacle(getRow(), getColumn(), number, side, game);
   }
   public int getRow()
   {
      return row;
   ...
   public int touches( Light light)
   {
      ...
      if( number == 3)
      {
         if( Math.sqrt( (light.endy-getYCenter())*(light.endy-getYCenter()) +  (light.endx-getXCenter())*(light.endx-getXCenter()))< 25 )
         {      
            return 0;  
         } 
         else return -1;
